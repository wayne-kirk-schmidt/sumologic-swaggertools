...
print('{},{},{}'.format('tag_group', 'tag_name', 'info_url'))
for keypath in benedict.keypaths(yaml_dict):
    if re.match(r"tags$", keypath):
        for tag_dict in yaml_dict[keypath]:
            tag_group = tag_dict['name']
            description = tag_dict['description']
            lines = description.split('. ')
            for line in lines:
                url_matches = re.search(r'(\[.*\])(\(.*\).)', line, re.DOTALL)
                if url_matches:
                    http_label = re.findall(r'\[(.*)\]', url_matches.groups()[0])
                    tag_name = http_label[0]
                    http_url = re.findall(r'\((.*)\)', url_matches.groups()[1])
                    tag_url = http_url[0]
            print('{},{},{}'.format(tag_group, tag_name, tag_url))
